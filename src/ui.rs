pub mod update {
    ...
    use cursive::{theme::{BaseColor, Color}, utils::markup::StyledString, views::{LinearLayout, SelectView}};
    use cursive::{
    ...
    pub fn file_list_view(s: &mut Cursive, file_list: Vec<FileEntry>) {
        s.call_on_name("command_layout", |layout: &mut LinearLayout| {
            ...
            select.add_all(file_list.into_iter().map(|file_entry| {
                match file_entry.filetype {
                    FileType::Directory => {
                        (StyledString::styled(file_entry.filename.clone(), Color::Light(BaseColor::Blue)), file_entry.filename)
                    },
                    FileType::Executable => {
                        (StyledString::styled(file_entry.filename.clone(), Color::Light(BaseColor::Green)), file_entry.filename)
                    },
                    _ => {
                        (StyledString::plain(file_entry.filename.clone()), file_entry.filename)
                    },
                }
            }));
